const keyBind = {'A':'a','B':'b','C':'c','D':'d','E':'e','F':'f','G':'g','H':'h','I':'i','J':'j','K':'k','L':'l','M':'m','N':'n','O':'o','P':'p','Q':'q','R':'r','S':'s','T':'t','U':'u','V':'v','W':'w','X':'x','Y':'y','Z':'z','F1':'f1','F2':'f2','F3':'f3','F4':'f4','F5':'f5','F6':'f6','F7':'f7','F8':'f8','F9':'f9','F10':'f10','F11':'f11','F12':'f12','+':'plus',' ':'space','ArrowLeft':'left','ArrowUp':'up','ArrowRight':'right','ArrowDown':'down','Shift':'shift','CapsLock':'capslock','Alt':'alt','AltGraph':'altgr','Backspace':'backspace','Delete':'delete','Tab':'tab','Escape':'escape','End':'end','Dead':'dead','Control':'ctrl','Insert':'insert','Enter':'enter','Meta':'meta','Home':'home','OS':'home','Pause':'break','PageUp':'pageup','PageDown':'pagedown','NumLock':'numlock','Cancel':'cancel','Clear':'clear'};

const deadKeys = ['f1','f2','f3','f4','f5','f6','f7','f8','f9','f10','f11','f12', 'left','up','right','down','shift','capslock','alt','altgr','backspace','delete','tab','escape','end','dead','ctrl','insert','enter','meta','home','break','pageup','pagedown','numlock','cancel','clear'];
...
const commandActions = {
    ...
    "left": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            if(caret.previousSibling != null) {
                if(caret.innerHTML == "")
                    area.insertBefore(caret, caret.previousSibling);
            }
            if(caret.innerHTML != "") {
                outContent(caret, "afterend");
            }
        });
    },
    ...
    "right": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            if(caret.nextSibling != null) {
                if(caret.innerHTML == "")
                    area.insertBefore(caret.nextSibling, caret);
            }
            if(caret.innerHTML != "") {
                outContent(caret, "beforebegin");
            }
        });
    },
    ...
    "up": (area, event) => {
        ...
        area.querySelectorAll("ide-caret").forEach(caret => {
            ...
            if(enterLvl == 1)
                area.insertBefore(caret, currentNode);

            if(enterLvl == 2) {
                area.insertBefore(caret, currentNode);
                area.insertBefore(currentNode, caret);
            }
        ...
        area.querySelectorAll("ide-caret").forEach(caret => {
                ...
                if(caret.previousSibling != null) {
                    if(caret.previousSibling.tagName == "IDE-CARET") {
                        while(caret.previousSibling.lastChild) {
                            ...
                            caret.insertAdjacentHTML("afterbegin", clonedChild);
                        }
                        area.removeChild(caret.previousSibling);
                    } else {
                        let clonedSibling = caret.previousSibling.textContent;
                        area.removeChild(caret.previousSibling);
                        caret.insertAdjacentHTML("afterbegin", clonedSibling);
                    }
                }
            }
            ...
            if(caret.classList.contains("right")) {
                if(caret.lastChild != null) {
                    ...
                    caret.insertAdjacentHTML("afterend", clonedChild);
                }
            }
        });
    },

    "shift+right": (area, event) => {
        [...area.querySelectorAll("ide-caret")].reverse().forEach(caret => {
            if(caret.innerHTML == "")
                ...
                if(caret.nextSibling != null) {
                    if(caret.nextSibling.tagName == "IDE-CARET") {
                        while(caret.nextSibling.firstChild) {
                            ...
                            caret.insertAdjacentHTML("beforeend", clonedChild);
                        }
            ...
            if(caret.classList.contains("left")) {
                if(caret.firstChild != null) {
                    ...
                    caret.insertAdjacentHTML("beforebegin", clonedFirstChild);
                }
            }
        });
    },
    ...
    "enter": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            ...
            while(findEnter != null
                && findEnter != caret
                && findEnter.textContent.replaceAll(/ /gi, "").length == 0) {
            ...
            tab.forEach(t => {
                caret.insertAdjacentHTML("beforebegin", t);
            });
        });
    },

    "shift+enter": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            let findEnter = caret,
                tab       = [];
    ...
    "tab": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            if(caret.previousSibling != null
                ...
                if(tab.length == 0)
                    caret.insertAdjacentHTML("beforebegin", " ".repeat(area.parentNode.parentNode.lenTab));
    ...
    "shift+tab": (area, event) => {

    },

    "backspace": (area, event) => {
        area.querySelectorAll("ide-caret").forEach(caret => {
            ...
            if(select.type == "Caret") {
                if(area.contains(select.focusNode)) {
                    if(select.focusNode == area)
                        ...
                        area.insertBefore(caret, select.focusNode);
                        if(select.anchorOffset > 0)
                            area.insertBefore(select.focusNode, caret);
                    }
                }
            }
        }
...
function setColumn(caret) {
    ...
    caret.setAttribute("column", column);
}
...
function outContent(caret, lr) {
    if(lr == "beforebegin")
        while(caret.firstChild) {
            let clonedSibling = caret.firstChild.textContent;
            caret.insertAdjacentHTML(lr, clonedSibling);
            caret.removeChild(caret.firstChild);
        }
   else if(lr == "afterend")
        while(caret.lastChild) {
            caret.insertAdjacentHTML(lr, caret.lastChild.textContent);
            caret.removeChild(caret.lastChild);
        }
}
...
function clearSiblingCaret(caret) {
    if(caret.nextSibling != null && caret.nextSibling.tagName == "IDE-CARET") {
        if(caret.nextSibling.innerHTML == "") {
            outContent(caret.nextSibling, "beforebegin");
...
function keyBindCommand(event) {
    ...
    if(event instanceof MouseEvent) {
        keys.push([null, 'r', 'm', 'l'][event.which] + 'click' + (event.detail > 1 ? event.detail%3 : ''));
    }

    let command = keys.join("+");
    if(command in commandActions) {
        let attrs = commandActions[command](event.target, event);
...
class IDE extends HTMLElement {
    ...
    connectedCallback() {
        const counter = document.createElement("div");
            counter.setAttribute("id", "ide-counter");

        const cell = document.createElement("div");
            cell.setAttribute("class", "ide-cell");
            cell.style.setProperty("--color", "#f000f0");
            cell.style.setProperty("--font-weight", "900");

        counter.appendChild(cell);
        this.appendChild(counter);

        const textarea = document.createElement("div");
            textarea.setAttribute("id", "ide-textarea");

        const syntax = document.createElement("div");
            syntax.setAttribute("id", "ide-syntax");

        const line = document.createElement("div");
            line.setAttribute("class", "ide-line");
        ...
        area.addEventListener("focusout", async event => {
            area.classList.add("focusout");
        });

        area.addEventListener("focus", async event => {
        ...
        area.addEventListener("keydown", event => {
            ...
            if(!action) allCaret.forEach(caret => {
                cleanContent(caret);
                caret.insertAdjacentHTML("beforebegin", event.key);
            });
...
customElements.define('ide-caret', Caret, {extends: "span"});
customElements.define('ide-area', IDE);